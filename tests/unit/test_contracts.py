...
def test_valid_customer_passes():
    ...
    Customer.model_validate(
        {"customer_id": 1, "name": "Ana", "email": "ana@email.com", "country": "Colombia"}
    )
...
def test_customer_name_empty_raises():
    ...
    with pytest.raises(ValidationError):
        Customer.model_validate(
            {"customer_id": 1, "name": "", "email": "ana@email.com", "country": "Colombia"}
        )
...
def test_transaction_negative_amount_raises():
    ...
    with pytest.raises(ValidationError):
        Transaction.model_validate(
            {"transaction_id": 100, "customer_id": 1, "amount": -1, "date": "2025-01-01"}
        )
...
def test_transaction_none_amount_raises():
    ...
    with pytest.raises(ValidationError):
        Transaction.model_validate(
            {"transaction_id": 100, "customer_id": 1, "amount": None, "date": "2025-01-01"}
        )
...
def test_transaction_wrong_date_format_raises():
    ...
    with pytest.raises(ValidationError):
        Transaction.model_validate(
            {"transaction_id": 100, "customer_id": 1, "amount": 100.0, "date": "01-01-2025"}
        )
