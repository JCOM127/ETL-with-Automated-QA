...
@pytest.mark.unit
def test_completeness_clean_passes(sample_clean_customers_df, sample_clean_transactions_df):
...
@pytest.mark.unit
def test_completeness_dirty_fails(sample_dirty_customers_df, sample_dirty_transactions_df):
...
def test_referential_integrity_clean_passes(
    ...
    result = check_referential_integrity(sample_clean_transactions_df, sample_clean_customers_df)
    assert result.passed is True
...
def test_referential_integrity_dirty_fails(
    ...
    result = check_referential_integrity(sample_dirty_transactions_df, sample_clean_customers_df)
    assert result.passed is False
...
def test_reconciliation_totals_match_passes(spark):
    ...
    raw_customers = [{"customer_id": 1, "name": "Ana", "email": "ana@x.com", "country": "Colombia"}]
    raw_transactions = [{"transaction_id": 100, "customer_id": 1, "amount": 200.0, "date": "2025-01-01"}]
    aggregated_df = spark.createDataFrame([("Colombia", 200.0)], ["country", "total_amount"])
